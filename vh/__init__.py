...
def au_delivery(mny, au_1, au_2, au_4, au_5, au_6, au_7, au_10, au_12, au_25, au_26, au_45, username):
    sex = session.get("sex")
    ...
    sem.acquire()
    print(mny, sex, username, au_1, au_2, au_4, au_5, au_6, au_7, au_10, au_12, au_25, au_26, au_45, username)
    res = requests.post(base_vh_host_url + '/receive_au', json=action_dict)
    sem.release()
    return res.status_code, res.text

def get_action_units(mny_idxs, username):
...
def purse_au_pic(mny_idx, usr_name=None):
    if usr_name is None:
        usr_name = str(session.get("dir"))

    action_unit = get_action_units([mny_idx], usr_name)[0]
...
def purse_au_pics(usr_name=None):
    if usr_name is None:
        usr_name = str(session.get("dir"))
    dir_name = "./static/img/testers/" + str(usr_name)
    ...
    action_units = get_action_units(action_unit_idx, usr_name)
    out_dir = dir_name + '/gnt'
    for i in range(len(action_units)):
        ...
        status, vh_pic = au_delivery(action_unit_idx[i], au_1, au_2, au_4, au_5, au_6, au_7, au_10, au_12, au_25, au_26, au_45, usr_name)
        if status != 200:
            return
        vh_pic_url = base_vh_host_url + vh_pic
        vh_pic_filedata = urllib.request.urlopen(vh_pic_url)
